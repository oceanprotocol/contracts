...
describe("Swap Fees", () => {
 ...
 describe(" Pool with NO ocean token and market fee 0.1%", async ()=> {
  const swapFee = 1e12
 ...
 it("#4 - user3 deploys a new erc20DT, assigning himself as minter", async () => {
  ...
  it("#5 - user3 calls deployPool() and check ocean and market fee", async () => {
    ...
    expect(await bPool._swapOceanFee()).to.equal(1e12)
    expect(await bPool._swapMarketFee()).to.equal(swapMarketFee)
  ...
  it("#8 - user4 swaps some DT back to Ocean swapExactAmountIn", async () => {
    ...
    assert(
      parseInt(await daiContract.balanceOf(user4.address)) >
        parseInt(user4DAIbalance)
    );
  });
