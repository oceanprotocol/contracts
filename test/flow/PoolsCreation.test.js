...
describe("Pools Creation Flow", () => {
  ...
  describe("POOL #1: 2 Token pool with OCEAN token and market Fee at 0.1%, AmountIn only test", async () => {
    ...
    it("#6 - user4 performs 2 additional swaps from datatoken to ocean", async () => {
      ...
      await erc20Token
        .connect(user3)
        .transfer(user4.address, web3.utils.toWei("100"));
    ...
    it("#7 - user2(could be any user as long as recipient is marketFeeCollector) triggers exitPool() and collects fees for marketPlace (Ocean and DT)", async () => {
      // In this pool dt is index 0 and ocean is 1
      const dtIndex = 0;
      const oceanIndex = 1;
      ...
      const alreadyCollectedInOcean = await pool.feesCollectedMarket(oceanIndex)
      // Creating the arguments for exitPool()
      const tokens = [erc20Token.address, oceanAddress];
      const exitKind = MP_FEE_WITHDRAWAL;
      const userData = ethers.utils.defaultAbiCoder.encode(
      ...
      // if we know attemp to call with the older marketFeeCollector it reverts
      ...
      assert(
        ...
        ).toString() == (totalMarketFeeInOcean - alreadyCollectedInOcean).toString()
      );
