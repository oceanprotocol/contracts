...
// TODO: use a different user than BOB 
  ...
  describe("#2 - Exchange with baseToken(DAI) 18 Decimals and dataToken 18 Decimals RATE = 1 ", async () => {
    ...
    it("#8 - Bob should buy ALL DataTokens available(amount exchangeOwner approved) using the fixed rate exchange contract", async () => {
      ...
      // ALICE's BT balance hasn't increasead. 
       expect(await daiContract.balanceOf(alice.address)).to.equal(btAliceBeforeSwap)
    ...
    it("#9 - Bob sells ALL DataTokens he has", async () => {
      ...
      // BOB's DTbalance is zero, and BT increased as expected
      expect(await mockDT18.balanceOf(bob.address)).to.equal(0)
      expect(await daiContract.balanceOf(bob.address)).to.equal(btBobBalanceBeforeSwap.add(SwappedEvent[0].args.baseTokenSwappedAmount))
    ...
    it("#11 - Alice withdraws BT balance available on the FixedRate contract", async () => {
      ...
      expect(Event[0].args.amount.add(btAliceBeforeSwap)).to.equal(await daiContract.balanceOf(alice.address))
    ...
    it("#13 - MarketFeeCollector withdraws fees available on the FixedRate contract", async () => {
      ...
      // marketFeeCollector balance (in Ocean is zero)
      const btMFCBeforeSwap = await daiContract.balanceOf(marketFeeCollector.address);
      ...
      // balance in ocean was transferred
      expect(await daiContract.balanceOf(marketFeeCollector.address)).to.equal(btMFCBeforeSwap.add(Event[0].args.feeAmount))
    ...
    it("#16 - Bob sells again some of his DataTokens", async () => {
      ...
      const btAliceBeforeSwap = await daiContract.balanceOf(alice.address);
      const dtAliceBeforeSwap = await mockDT18.balanceOf(alice.address);
      expect(btAliceBeforeSwap).to.equal(web3.utils.toWei('2005')) // Alice(owner) has no BT 
    ...
    it("#17 - MarketFeeCollector updates new address then withdraws fees available on the FixedRate contract", async () => {
      ...
      const btMFCBeforeSwap = await daiContract.balanceOf(newMarketFeeCollector.address);
      ...
      // balance in ocean was transferred
      expect(await daiContract.balanceOf(newMarketFeeCollector.address)).to.equal(btMFCBeforeSwap.add(Event[0].args.feeAmount))
  ...
  describe("#3 - Exchange with baseToken(OCEAN) 18 Decimals and dataToken 18 Decimals, RATE = 2  (2 OCEAN = 1 DT)", async () => {
    ...
    //  expect(btAliceBeforeSwap).to.equal(0) // Alice(owner) has no BT 
      ...
      expect(
        args.baseTokenSwappedAmount.sub(args.oceanFeeAmount).sub(args.marketFeeAmount)).to.equal(args.dataTokenSwappedAmount.mul(rateX))
    ...
    //  expect(btAliceBeforeSwap).to.equal(0) // Alice(owner) has no BT 
      ...
      // we check that proper amount is being swapped (rate=2)
      expect(
       args.baseTokenSwappedAmount.add(args.oceanFeeAmount).add(args.marketFeeAmount)).to.equal(args.dataTokenSwappedAmount.mul(rateX))
    ...
    it("#12 - Bob buys back all DT left (80%) of DataTokens available", async () => {
      ...
      // we check that proper amount is being swapped (rate=1)
      expect(
        SwappedEvent[0].args.baseTokenSwappedAmount.sub(args.marketFeeAmount).sub(args.oceanFeeAmount)).to.equal(SwappedEvent[0].args.dataTokenSwappedAmount.mul(rateX))
    ...
    it("#15 - Bob sells again some of his DataTokens", async () => {
      ...
      // we check that proper amount is being swapped (rate=1)
      expect(
        SwappedEvent[0].args.baseTokenSwappedAmount.add(args.marketFeeAmount).add(args.oceanFeeAmount)).to.equal(SwappedEvent[0].args.dataTokenSwappedAmount.mul(rateX))
  ...
  xdescribe("#4 - Exchange with baseToken(DAI) 18 Decimals and dataToken 18 Decimals, RATE = 2  (2 DAI = 1 DT)", async () => {
    let maxAmountBTtoSell = web3.utils.toWei("100000") , // bigger than required amount
    amountDTtoSell = web3.utils.toWei("10000") // exact amount so that we can check if balances works
    ...
    it("#8 - Bob should buy ALL DataTokens available(amount exchangeOwner approved) using the fixed rate exchange contract", async () => {
      ...
      const btAliceBeforeSwap = await daiContract.balanceOf(alice.address);
      expect(dtBobBalanceBeforeSwap).to.equal(0) // BOB HAS NO DT
      expect(btAliceBeforeSwap).to.equal(web3.utils.toWei('5')) // Alice(owner) has only 5 DAI
    ...
    it("#12 - Bob buys back all DT left (80%) of DataTokens available", async () => {
      ...
      const btAliceBeforeSwap = await daiContract.balanceOf(alice.address);
      expect(dtBobBalanceBeforeSwap).to.equal(web3.utils.toWei('2000')) // BOB HAS 20% of initial DT available
      expect(btAliceBeforeSwap).to.equal(web3.utils.toWei('4005')) // Alice(owner) has already withdrew her BT (#11)
    ...
    it("#15 - Bob sells again some of his DataTokens", async () => {
      ...
      const btAliceBeforeSwap = await daiContract.balanceOf(alice.address);
      const dtAliceBeforeSwap = await mockDT18.balanceOf(alice.address);
      expect(btAliceBeforeSwap).to.equal(web3.utils.toWei('4005')) // Alice(owner) has already withdrawn her BT
  ...
  xdescribe("#5 - Exchange with baseToken(USDC) 6 Decimals and dataToken 18 Decimals, RATE = 2  (2 USDC = 1 DT)", async () => {
    ...
    it("#8 - Bob should buy ALL DataTokens available(amount exchangeOwner approved) using the fixed rate exchange contract", async () => {
      ...
      // we check that proper amount is being swapped (rate=2)
      expect(
        args.baseTokenSwappedAmount.sub(args.oceanFeeAmount).sub(args.marketFeeAmount).mul(1e12)).to.equal(args.dataTokenSwappedAmount.mul(rateX))
    ...
    it("#9 - Bob sells ALL DataTokens he has", async () => {
      ...
      // we check that proper amount is being swapped (rate=2)
      expect(
       args.baseTokenSwappedAmount.add(args.oceanFeeAmount).add(args.marketFeeAmount).mul(1e12)).to.equal(args.dataTokenSwappedAmount.mul(rateX))
    ...
    it("#12 - Bob buys back all DT left (80%) of DataTokens available", async () => {
      ...
      // we check that proper amount is being swapped (rate=2)
      expect(
        SwappedEvent[0].args.baseTokenSwappedAmount.sub(args.marketFeeAmount).sub(args.oceanFeeAmount).mul(1e12)).to.equal(SwappedEvent[0].args.dataTokenSwappedAmount.mul(rateX))
    ...
    it("#15 - Bob sells again some of his DataTokens", async () => {
      ...
      // we check that proper amount is being swapped (rate=1)
      expect(
        SwappedEvent[0].args.baseTokenSwappedAmount.add(args.marketFeeAmount).add(args.oceanFeeAmount).mul(1e12)).to.equal(SwappedEvent[0].args.dataTokenSwappedAmount.mul(rateX))
