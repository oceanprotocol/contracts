...
describe("ERC721Factory", () => {
  ...
  it("#deployERC721Contract - should fail if token template is not active", async () => {
    const tx = await factoryERC721.add721TokenTemplate(
  ...
  it("#add721TokenTemplate - should fail to add Token Template if not Owner", async () => {
    await expectRevert(
      factoryERC721
        .connect(user2)
        .add721TokenTemplate(newERC721Template.address),
  ...
  it("#createToken - should not allow to create a new ERC20Token if NOT in CreateERC20List", async () => {
    assert(
      (await tokenERC721.getPermissions(user6.address)).deployERC20 == false
    );
    await expectRevert(
      tokenERC721
        .connect(user6)
  ...
  it("#startMultipleTokenOrder - user should succeed to call startOrder on a ERC20 without publishFee", async () => {
    ...
    const tx = await factoryERC721.connect(user2).startMultipleTokenOrder([
      {
  ...
  it("#startMultipleTokenOrder - user should succeed to call startOrder on a ERC20 without publishFee", async () => {
    ...
    const signedMessage = await signMessage(message, providerFeeAddress);
    await erc20Token.connect(user2).approve(factoryERC721.address, dtAmount);
    console.log("4");
    const tx = await factoryERC721.connect(user2).startMultipleTokenOrder([
      {
        tokenAddress: erc20Token.address,
  ...
  it("#startMultipleTokenOrder - user should succeed to call startOrder on a ERC20 without publishFee", async () => {
    ...
    assert(
      (await erc20Token.balanceOf(await erc20Token.getPaymentCollector())) ==
        web3.utils.toWei("1"),
      "Invalid publisher reward, he should get 1 DT"
    );
  });
  ...
  it("#createNftWithErc20WithPool - should create a new erc721 and new erc20 and a new Pool in one single call and get their addresses", async () => {
    ...
    await erc20Token
      .connect(user3)
      .approve(factoryERC721.address, initialPoolLiquidy);
    ...
    assert(
      (await poolContract.balanceOf(user3.address)) > 0,
      "Invalid pool share #"
    );
  });
