...
/**
 * @title Enterprise Escrow contract
 *
    ...
    ReentrancyGuard
{
    ...
    // EnterpriseFeeCollector address
    ...
    constructor(address _opcCollector) {
    ...
    // fee out last. fee is charged on payoutBase. The fee is CAPPED at payoutBase: a (trusted but
    // possibly misconfigured) collector returning fee >= payoutBase must neither underflow-revert the
    // claim (DoS) NOR cause the escrow to transfer out more than this claim's base (which would dip
    // the shared pool and break solvency). Capping preserves both liveness and solvency; the node
    // simply receives 0 and the collector receives at most payoutBase.
    // slither-disable-next-line reentrancy-eth,reentrancy-benign,reentrancy-events
    function _creditPayout(address token,uint256 payoutBase) internal {
        uint256 payout = payoutBase;
        if(opcCollector != address(0)){
            uint256 feeAmount = IEnterpriseFeeCollector(opcCollector).calculateFee(token,payoutBase);
            if(feeAmount > payoutBase) feeAmount = payoutBase; // cap: never pay out more than the base
            payout = payoutBase - feeAmount;
