...
/**
 * @title FixedRateExchangeEnterprise
 * @dev FixedRateExchangeEnterprise is a fixed rate exchange Contract
 *      Marketplaces uses this contract to allow consumers
 *      exchanging datatokens with ocean token using a fixed
 *      exchange rate.
 *      In addition to FixedRateExchange, it takes an additional fee for OPE
 */



contract FixedRateExchangeEnterprise is ReentrancyGuard {
    ...
    constructor(address _router, address _enterpriseFeeCollector) {
        require(_router != address(0), "FixedRateExchange: Wrong Router address");
        router = _router;
        enterpriseFeeCollector = _enterpriseFeeCollector;
    }
    ...
    function getOPEFee(address baseTokenAddress,uint256 amount) public view returns (uint) {
        // TO DO
        if(enterpriseFeeCollector == address(0)){ return 0;}
    return(
        IEnterpriseFeeCollector(enterpriseFeeCollector).calculateFee(
            baseTokenAddress,
            amount
        ));
    }
        ...
        nonReentrant
    {
        ...
        Fees memory fee = calcBaseInGivenOutDT(exchangeId, datatokenAmount, consumeMarketSwapFeeAmount);
        require(
